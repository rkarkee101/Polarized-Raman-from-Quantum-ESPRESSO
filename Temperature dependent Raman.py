...
temperatures = [6, 100, 200, 300]  # Add more temperatures as needed

# Create a figure for plotting
plt.figure(figsize=(10, 5))

# Loop through each temperature and generate the plot
for temp in temperatures:
    ...
    boson_occ = boson_number(exp, temp)
    boson_occ[0:3] = 0
    ...
    for gamma in np.arange(0, 90, 90):
        ...
        # Plot the smoothed Raman spectrum for the current temperature
        plt.plot(x_smoothed, smoothed_intensity_xz, label=f'T={temp}K', linestyle='-')
...
plt.legend()
plt.title('Raman Spectra at Different Temperatures')
plt.grid(True)
plt.show()
plt.savefig('Temp dependent Raman XZ.png', dpi=600, bbox_inches='tight')
